...
{
	int a,b,c,d;
	// When we set EAX=0x80000001 and run CPUID instruction, the returning value in bit 2 of ECX indicates whether or not svm is supported.
	cpuid(0x80000001,a,b,c,d);
	if(c & 1<<ECX_BIT_SVM)
	{
		DPRINTF("DEBUG: Feature: Nested SVM exists!\n");
		return 1;
	}else
	{
		DPRINTF("DEBUG: Feature: Nested SVM not exists!\n");
		return 0;
	}
	return 0;
}
