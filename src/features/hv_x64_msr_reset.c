...
// Test if Hyper-V MSR HV_X64_MSR_RESET is readable
int test_hv_x64_msr_reset(){
    DPRINTF("Starting HV_X64_MSR_RESET test...\n");
    ...
    if (pid == 0) {
        ...
        rdmsr_on_cpu(HV_X64_MSR_RESET, 0);
        ...
        if (!WIFEXITED(status) || WEXITSTATUS(status) != 0) {
            ...
            DPRINTF("DEBUG: Parent: Feature not present: Cannot read HV_X64_MSR_RESET!\n");
            return 0;
        } else {
            // successful register read, so feature is present
            DPRINTF("DEBUG: Parent: Feature present: HV_X64_MSR_RESET is readable!\n");
            return 1;
        }
    }
}