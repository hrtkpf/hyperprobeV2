/*
 * This file implements a test case for check paravirtualized eoi enable feature.
 * Initial work by:
...
// Thie function use fork to create a child process. The child process tries to read MSR_KVM_PV_EOI_EN.
...
int test_pv_eoi_en()
{
	...
	if(pid==0)	//child process
	{
		...
		rdmsr_on_cpu(MSR_KVM_PV_EOI_EN,0);
		DPRINTF("DEBUG: Child: Feature Exists: MSR_KVM_PV_EOI_EN is readable!\n");
		exit(0);
	}else		//parent process
	{
		wait(&status);
                if(WIFEXITED(status))	// WIFEXITED(status) returns true if the child terminated normally
                {
                        if(WEXITSTATUS(status))	// WEXITSTATUS(status) returns  the  exit  status  of the child.
                        {
                                ...
                                DPRINTF("DEBUG: Parent: Feature not Exists: MSR_KVM_PV_EOI_EN is not readable!\n");
                                return 0;
                        }
                        else
                        {
                                DPRINTF("DEBUG: Parent: The return code of child process is zero.\n");
                                DPRINTF("DEBUG: Parent: Feature Exists: MSR_KVM_PV_EOI_EN is readable!\n");
