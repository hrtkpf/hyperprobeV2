...
//		printf("DEBUG: Child: %s %d \n",__FUNCTION__,__LINE__);
	...
	}else		//parent process
	{
		...
		if(WIFEXITED(status))
		{
			return 1;	//child process exit normally, which means the register is readable, so the feature is existing.
		}else
		{
			return 0;	//child process exit abnormally, the register is not readable, so we are not sure whether the feature exist or not.
		}
