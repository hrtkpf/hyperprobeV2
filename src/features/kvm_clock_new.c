/*
 * This file implements a test case for check kvm clock new feature.
 * Initial work by:
...
int test_kvm_clock_new()
{
	...
	if(pid==0)	//child process
	{
		...
		rdmsr_on_cpu(MSR_KVM_WALL_CLOCK_NEW,0);
		DPRINTF("DEBUG: Child: Feature Exists: MSR_KVM_WALL_CLOCK_NEW is readable!\n");
		exit(0);
	}else		//parent process
	{
		wait(&status);
                if(WIFEXITED(status))	// WIFEXITED(status) returns true if the child terminated normally
                {
                        if(WEXITSTATUS(status))	// WEXITSTATUS(status) returns  the  exit  status  of the child.
                        {
                                ...
                                DPRINTF("DEBUG: Parent: Feature not Exists: MSR_KVM_WALL_CLOCK_NEW is not readable!\n");
                                return 0;
                        }
                        else
                        {
                                DPRINTF("DEBUG: Parent: The return code of child process is zero.\n");
                                DPRINTF("DEBUG: Parent: Feature Exists: MSR_KVM_WALL_CLOCK_NEW is readable!\n");
