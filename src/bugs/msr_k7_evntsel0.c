...
int test_msr_k7_evntsel0()
{
	...
	if(pid==0)	//child process
	{
		...
		rdmsr_on_cpu(MSR_K7_EVNTSEL0,0);
		DPRINTF("DEBUG: Child: Bug Fixed: MSR_K7_EVNTSEL0 is readable!\n");
		exit(0);
	}else		//parent process
	{
		wait(&status);
			...
			if(WEXITSTATUS(status))
                        {
                                ...
                                DPRINTF("DEBUG: Parent: Bug Exists: MSR_K7_EVNTSEL0 is not readable!\n");
                                return 1;
                        }
                        else
                        {
                                DPRINTF("DEBUG: Parent: The return code of child process is zero.\n");
                                DPRINTF("DEBUG: Parent: Bug Fixed: MSR_K7_EVNTSEL0 is readable!\n");
