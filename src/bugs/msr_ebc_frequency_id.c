/*
 * This file implements a test case for check the MSR_EBC_FREQUENCY_ID bug.
 * Initial work by:
...
// Thie function use fork to create a child process. The child process tries to read MSR_EBC_FREQUENCY_ID.
// If the register exists, it is readable. Otherwise, it is not readable.
// Return: 1 if feature exist, 0 if not sure.
int test_msr_ebc_frequency_id()
{
	pid_t pid;
	int status;
	...
	if(pid==0)	//child process
	{
		...
		rdmsr_on_cpu(MSR_EBC_FREQUENCY_ID,0);
		DPRINTF("DEBUG: Child: Bug Fixed: MSR_EBC_FREQUENCY_ID is readable!\n");
		exit(0);
	}else		//parent process
	{
		wait(&status);
		if(WIFEXITED(status))
		{
			if(WEXITSTATUS(status))
			{
				...
				DPRINTF("DEBUG: Parent: Bug Exists: MSR_EBC_FREQUENCY_ID is not readable!\n");
				return 1;
			}
			else
			{
				DPRINTF("DEBUG: Parent: The return code of child process is zero.\n");
				DPRINTF("DEBUG: Parent: Bug Fixed: MSR_EBC_FREQUENCY_ID is readable!\n");
