...
int test_msr_ia32_mc0_misc5() {
    ...
    if (pid == 0)    //child process
    {
        ...
        rdmsr_on_cpu(MSR_IA32_MC0_MISC + 20, 0);
        DPRINTF("DEBUG: Child: Bug Fixed: MSR_IA32_MC0_MISC+20 is readable!\n");
        exit(0);
    } else        //parent process
    {
        wait(&status);
        if (WIFEXITED(status)) {
            if (WEXITSTATUS(status)) {
                ...
                DPRINTF("DEBUG: Parent: Bug Exists: MSR_IA32_MC0_MISC+20 is not readable!\n");
                return 1;
            } else {
                DPRINTF("DEBUG: Parent: The return code of child process is zero.\n");
                DPRINTF("DEBUG: Parent: Bug Fixed: MSR_IA32_MC0_MISC+20 is readable!\n");
