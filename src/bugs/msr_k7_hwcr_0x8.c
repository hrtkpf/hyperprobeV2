/*
 * This file implements a test case for check the MSR_K7_HWCR writing 0x8 bug.
 * Initial work by:
...
// Thie function use fork to create a child process. The child process tries to write 0x8 to MSR_K7_HWCR.
// If the register exists, it is readable. Otherwise, it is not readable.
// Return: 1 if feature exist, 0 if not sure.
int test_msr_k7_hwcr_0x8()
{
	pid_t pid;
	int status;
	...
	if(pid==0)	//child process
	{
		...
		wrmsr_on_cpu(MSR_K7_HWCR,0,0x8);
		DPRINTF("DEBUG: Child: Bug Fixed: MSR_K7_HWCR 0x8 is writable!\n");
		exit(0);
	}else		//parent process
	{
		wait(&status);
		if(WIFEXITED(status))
		{
                        if(WEXITSTATUS(status))
                        {
                                ...
                                DPRINTF("DEBUG: Parent: Bug Exists: writing 0x8 to MSR_K7_HWCR is not allowed!\n");
                                return 1;
                        }
                        else
                        {
                                DPRINTF("DEBUG: Parent: The return code of child process is zero.\n");
                                DPRINTF("DEBUG: Parent: Bug Fixed: writing 0x8 to MSR_K7_HWCR is allowed!\n");
