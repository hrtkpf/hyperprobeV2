...
int test_msr_ia32_ebl_cr_poweron()
{
	pid_t pid;
	int status;
	...
	if(pid==0)	//child process
	{
		...
		rdmsr_on_cpu(MSR_IA32_EBL_CR_POWERON,0);
		DPRINTF("DEBUG: Child: Bug Fixed: MSR_IA32_EBL_CR_POWERON is readable!\n");
		exit(0);
	}else		//parent process
	{
		wait(&status);
		if(WIFEXITED(status))
		{
			if(WEXITSTATUS(status))
			{
				...
				DPRINTF("DEBUG: Parent: Bug Exists: MSR_IA32_EBL_CR_POWERON is not readable!\n");
				return 1;
			}
			else
			{
				DPRINTF("DEBUG: Parent: The return code of child process is zero.\n");
				DPRINTF("DEBUG: Parent: Bug Fixed: MSR_IA32_EBL_CR_POWERON is readable!\n");
